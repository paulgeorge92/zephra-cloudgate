...
import { ArrowLeft, ExternalLink, Globe, Lock, Route, Server, ShieldCheck, Waypoints, } from 'lucide-react';
import AppLayout from '@/components/layout/AppLayout';
import { getAccessApp, getApplication, getTunnel } from '@/lib/api';
import { Application, Tunnel, HealthStatus } from '@shared/types';
import { ApplicationExposureTypeEnum } from '@shared/types'
import { CloudflareAccessApplication, CloudflareAccessPolicy } from '@cloudflare';
...
const healthConfig: Record<string, { label: string; className: string; dot: string }> = {
  CHECKING: {
    label: 'Checking',
...
export default function ApplicationDetailPage() {
  ...
  const healthStyle = healthConfig[health.status] || healthConfig['UNKNOWN'];
  ...
  const fetchApplication = async () => {
    ...
    try {
      const appData = (await getApplication(params.id)).data.result as ApplicationDetail;
      setApp((appData|| null) as ApplicationDetail | null);
      setHealth(appData?.health || {
        status: 'UNKNOWN',
        message: 'Unable to fetch health status',
        statusCode: undefined
      });
      fetchTunnel(appData)
      fetchAppPolicy(appData)
    } catch (err) {
  ...
  const fetchTunnel = async (appData: ApplicationDetail) => {
    if (!appData || !appData.tunnelId) return;
    const tunnel = await getTunnel(appData.tunnelId);

    if (tunnel.data.result) {
      const _data = tunnel.data.result as Tunnel;
      setApp({ ...appData, tunnel: _data } as ApplicationDetail)
    }
  }

  const fetchAppPolicy = async (appData: ApplicationDetail) => {
    if (!appData) return;
    const accessApp = await getAccessApp(appData.id);
    if (accessApp.data.result) {
      const policy = (accessApp.data.result as CloudflareAccessApplication).policies?.[0] as CloudflareAccessPolicy;
      if (policy) {
        setApp({
          ...appData, policy: {
            policyId: policy.id,
          ...
          <>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
              ...
              <section className="xl:col-span-2 glass-panel p-6">
                ...
                {app.tunnel ? (
                  <Link href={`/tunnels/${app.tunnel.cfTunnelId}`}>
                    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4">
