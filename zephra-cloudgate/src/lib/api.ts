...
import { User, Application, DashboardSummary, SystemStats, WeatherStats, Tunnel, TunnelDetail, Device, FirewallPolicy, SetupStatus, ServerProfile, SetupAdminData, CloudflareConfigData, SmtpConfigData, CreateApplicationData, CreateUserData, UpdateUserData, IngressRule, AuthResponse, RequestResponse, AccessApp, DevicePostureRule, FirewallContentCategory, ReusableList, DiskStats, SetupVerificationResult } from '@shared/types';
import {CloudflareAccessApplication} from '@cloudflare'
const api = axios.create({
  headers: { 'Content-Type': 'application/json' }
});

// Attach JWT token to same-origin API requests.
...
export const getAccessApps = () => api.get<RequestResponse<CloudflareAccessApplication[]>>('/api/v1/cloudflare/access/apps');
export const getAccessApp=(id:string) => api.get<RequestResponse<CloudflareAccessApplication>>(`/api/v1/cloudflare/access/apps/${id}`);
