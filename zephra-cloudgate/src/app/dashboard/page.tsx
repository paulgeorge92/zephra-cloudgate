...
import { DashboardSummary, SystemStats, WeatherStats, User as AppUser, DiskStats } from '@shared/types';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Initial render only waits for the data needed to draw the main shell.
    setLoading(true);
    Promise.all([
      getDashboard(),
      getSystemStats(),
      getMe()
    ]).then(([statsRes, systemRes, userRes]) => {
      if (cancelled) return;
    ...
    getStorageStats()
      .then((storageRes) => {
        if (cancelled) return;
        const result = storageRes.data?.result || [];
        setStorage(result);
        if (result.length === 0) {
          window.setTimeout(async () => {
            if (cancelled) return;
            try {
              const retryRes = await getStorageStats();
              if (!cancelled) setStorage(retryRes.data?.result || []);
            } catch (err) {
    ...
    getWeather()
      .then((weatherRes) => {
        if (!cancelled) setWeather(weatherRes.data?.result || null);
      })
