...
import { CloudflareTunnelConfig, CloudflareAccessApplication, CloudflareMember, CloudflareResponse, CloudflareNetworkRoute, CloudflareAccountRole, CloudflareZone, CloudflareTunnelWithToken, CloudflareTunnel, CloudflareDevice, CloudflareTunnelConnectionDetail, CloudflareDNSRecord, CloudflareAccessPolicy, CloudflareContentCategory, CloudflareDevicePostureRule, CloudflareGatewayRule, CloudflareVerificationResult, CloudflareTrafficLog, CloudflareHTTPLog, CloudflareDNSLog, CloudflareNetworkLog, CloudflareIngressRule } from './cloudflare.types';
import { ApplicationExposureTypeEnum, FirewallAppType, ReusableList } from '../shared/types';
...
export class CloudflareService {
  ...
  async getAccessApps(): Promise<CloudflareResponse<CloudflareAccessApplication[]>> {
    const { accountId } = await this.getClient();
    return this.apiRequest<CloudflareAccessApplication[]>('GET', `/accounts/${accountId}/access/apps`, null, { per_page: 100 });
  }

  async getAccessApp(id: string): Promise<CloudflareResponse<CloudflareAccessApplication>>{
    const { accountId } = await this.getClient();
    return this.apiRequest<CloudflareAccessApplication>('GET', `/accounts/${accountId}/access/apps/${id}`);
  }
  ...
  async getAccessPolicy(policyId: string): Promise<CloudflareResponse<CloudflareAccessPolicy>> {
    const { accountId } = await this.getClient();
    const res = await this.apiRequest<CloudflareAccessPolicy>('GET', `/accounts/${accountId}/access/policies/${policyId}`);
